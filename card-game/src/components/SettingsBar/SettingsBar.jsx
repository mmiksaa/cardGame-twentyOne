...
import { toggleGameCounter, changeTheme, toggleJDK, toggleHideHud } from '../../redux/cardsSlice';
  ...
  return (
    <>
      <button
        ...
        <img src={settingsImg} alt="" style={{ maxWidth: '50px' }} />
      </button>
...
const SettingsPopup = ({ showPopup, onShowPopup, popupRef, counterItems }) => {
  ...
  const { gameCounterMain, theme, hideJDK, hideHUD } = useSelector((state) => state.cards.settings);

  useEffect(() => {
    theme === 'white' ? document.body.classList.add(`theme-${theme}`)
  ...
  return (
    <div
      ref={popupRef}
      className={classNames('popup settings-popup fade', { 'popup--show': showPopup })}>
      ...
      <ul>
        ...
        <li className="settings-popup__item">
          <div className="settings-popup__label settings-popup__label--line clear-bar">
            <div></div>
            <p className="settings-popup__text settings-popup__text--line">
              clear the counting bar
            </p>
          </div>
        </li>

        <li className="settings-popup__item">
          <div className="settings-popup__label settings-popup__label--line">
            <div></div>
            <p className="settings-popup__text settings-popup__text--line">
              clear the counting game
            </p>
          </div>
        </li>
      </ul>
