import { SettingsBar, Instruction, Table, GameBar, EndGame, GithubSvg } from './components';
import { useSelector } from 'react-redux';
import classNames from 'classnames';

function App() {
  const gameCounter = useSelector(state => state.cards.gameCounter);
  const { gameCounterMain, hideHUD } = useSelector(state => state.cards.settings);
  ...
  return (
    <div className="wrapp">
      ...
      <GithubSvg hideHUD={hideHUD}className="github" />
    </div>
  );
}
